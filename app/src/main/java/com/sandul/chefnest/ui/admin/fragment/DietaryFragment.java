...
public class DietaryFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        Button cuisine_button = getView().findViewById(R.id.dietary_button);
        cuisine_button.setOnClickListener(v -> {
            EditText cuisine_name = getView().findViewById(R.id.editText_dietary);
    ...
    private void loadCuisineData() {
        new Thread(() -> {
            try {
                JsonObject responseJsonObject = NetworkUtils.makeGetRequest("/load-dietary");
                ...
                if (responseJsonObject != null && responseJsonObject.get("message").getAsString().equals("success")) {
                    getActivity().runOnUiThread(() -> {
                        ...
                        responseJsonObject.getAsJsonArray("dietaries").forEach(type -> {
                        ...
                        RecyclerView recyclerView = getView().findViewById(R.id.dietaries_view);
                    ...
                    getActivity().runOnUiThread(() -> {
                        Toast.makeText(getContext(), "Failed to load dietary data", Toast.LENGTH_SHORT).show();
                    });
                }
            } catch (IOException e) {
                e.printStackTrace();
    ...
    private void addCuisine(String dietaryName) {
        new Thread(() -> {
            try {
                ...
                body.addProperty("dietaryName", dietaryName);
                JsonObject responseJsonObject = NetworkUtils.makePostRequest("/admin/add-dietary", body.toString());
                ...
                if (responseJsonObject != null && responseJsonObject.get("message").getAsString().equals("success")) {
                    getActivity().runOnUiThread(() -> {
                        ...
                        Toast.makeText(getContext(), "Dietary added successfully", Toast.LENGTH_SHORT).show();
                    });
                } else {
                    getActivity().runOnUiThread(() -> {
                        Toast.makeText(getContext(), "Failed to add dietary", Toast.LENGTH_SHORT).show();
                    });
                }
            } catch (IOException e) {
                e.printStackTrace();
